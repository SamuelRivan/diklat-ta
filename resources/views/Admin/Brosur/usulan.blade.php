...
<div class="container mt-4">
    <h2 class="mb-3">Daftar Usulan Brosur</h2>

    <!-- Tombol Tambah Data -->
    <a href="{{ route('Admin.Brosur.create') }}" class="btn btn-success mb-3">
        <i class="fa fa-plus"></i> Tambah Data
    </a>

    <!-- Form Pencarian -->
    <form method="GET" action="{{ route('Admin.Brosur.usulan') }}" class="row g-3 mb-3">
        <div class="col-md-6">
            <input type="text" name="search" value="{{ request('search') }}" class="form-control" placeholder="Cari penyelenggara...">
        </div>
        <div class="col-md-4">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ ($brosur_2_masuks->firstItem() ?? 0) + $loop->iteration - 1 }}</td>
                    ...
                    <td>
                        <div class="btn-group" role="group">
                            Tombol Edit
                            <a href="{{ route('Admin.Brosur.editusulan', $item->getKey()) }}" class="btn btn-sm btn-primary">
                                <i class="fa fa-edit"></i>
                            </a>

                            <!-- Tombol Hapus -->
                            <form action="{{ route('brosur.deleteusulan', ['id' => $item->id]) }}" method="POST"
                            ...
                            <!-- Tombol Approve -->
                            <form action="{{ route('brosur.approve', $item->getKey()) }}" method="POST"
                                onsubmit="return confirm('Yakin ingin menyetujui?')">
                                @csrf
                                @method('PUT') {{-- Perbaiki cara penulisan ini --}}
