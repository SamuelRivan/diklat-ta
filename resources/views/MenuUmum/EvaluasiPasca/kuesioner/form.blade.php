{{-- View Form Pengisian Kuesioner Pascadiklat --}}
<!DOCTYPE html>
<html lang="id">
...
<body>
    ...
    <div class="container mt-4">
        ...
        <!-- Form Kuesioner -->
        <form action="{{ route('pascadiklat.kuesioner.store') }}" method="POST" id="kuesionerForm">
            @csrf
            <input type="hidden" name="kuesioner_id" value="{{ $kuesioner->id }}">
            @if($pelatihan)
            <input type="hidden" name="pelatihan_id" value="{{ $pelatihan->id }}">
            @endif
            <input type="hidden" name="role_pengisi" value="alumni">
            ...
            <div class="card pertanyaan-card">
                <div class="card-body">
                    ...
                    @elseif($item->jenis === 'radio' || $item->jenis === 'pilihan_ganda')
                        @foreach($item->opsiJawaban->sortBy('urutan') as $opsi)
